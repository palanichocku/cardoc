  ...
  return (
    <>
      <PageHeading
        eyebrow="Admin"
        title="Shop Settings"
        description="Manage invoice defaults and shop document messages."
      />
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <h2 className="text-lg font-semibold">Invoice defaults</h2>
        <p className="mt-2 text-sm leading-6 text-slate-600">These defaults apply only when a web-created repair order is finalized in the future.</p>
        {saved === "1" && <p className="mt-4 rounded-lg bg-emerald-50 px-4 py-3 text-sm font-medium text-emerald-800">Invoice defaults saved.</p>}
        <form action={updateInvoiceSettings} className="mt-6 max-w-2xl space-y-5">
          ...
          <button type="submit" className="rounded-lg bg-sky-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-sky-700">Save invoice defaults</button>
        </form>
      </section>
    </>
  );
