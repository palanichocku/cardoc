  ...
  return (
    <>
      <PageHeading
        ...
        title="Open Orders"
        description="Read-only repair orders that have not been finalized as invoices."
      />
      {orders.length === 0 ? (
        ...
        <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          <ul className="divide-y divide-slate-200">
            {orders.map((order: OpenOrder) => {
              ...
              return (
                <li key={order.id}>
                  <Link href={`/open-orders/${order.id}`} className="grid gap-2 px-5 py-4 transition hover:bg-slate-50 md:grid-cols-[0.8fr_1.2fr_1.2fr_1fr_0.8fr] md:items-center">
                    <span><span className="block font-semibold text-slate-950">RO #{order.legacyRoNo ?? "Not recorded"}</span><span className="text-sm text-slate-500">{formatDate(order.openedAt)}</span></span>
