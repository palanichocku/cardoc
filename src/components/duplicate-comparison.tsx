...
const confidenceClass = { High: "bg-emerald-100 text-emerald-800", Medium: "bg-amber-100 text-amber-800", Low: "bg-slate-100 text-slate-700" } as const;
...
export function DuplicateComparison({ customerGroups, vehicleGroups }: { customerGroups: Array<DuplicateGroup<CustomerDuplicate>>; vehicleGroups: Array<DuplicateGroup<VehicleDuplicate>> }) {
  ...
  const source = tab === "customers" ? customerGroups : vehicleGroups;
  const visible = useMemo(() => source.filter((group) => (!highOnly || group.confidence === "High") && (reason === "all" || group.reason.toLowerCase().includes(reason))), [source, highOnly, reason]);
  const toggle = (id: string) => setExpanded((current) => { const next = new Set(current); if (next.has(id)) next.delete(id); else next.add(id); return next; });

  return <>
    <div className="flex flex-wrap items-center gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      <div className="flex rounded-lg bg-slate-100 p-1" aria-label="Duplicate type">
        <button type="button" onClick={() => { setTab("customers"); setReason("all"); }} className={`rounded-md px-3 py-2 text-sm font-semibold ${tab === "customers" ? "bg-white text-sky-700 shadow-sm" : "text-slate-600"}`}>Customers ({customerGroups.length})</button>
        <button type="button" onClick={() => { setTab("vehicles"); setReason("all"); }} className={`rounded-md px-3 py-2 text-sm font-semibold ${tab === "vehicles" ? "bg-white text-sky-700 shadow-sm" : "text-slate-600"}`}>Vehicles ({vehicleGroups.length})</button>
      </div>
      <label className="flex items-center gap-2 text-sm font-medium text-slate-700"><input type="checkbox" checked={highOnly} onChange={(event) => setHighOnly(event.target.checked)} /> High confidence only</label>
      <label className="text-sm font-medium text-slate-700">Match reason <select value={reason} onChange={(event) => setReason(event.target.value as Reason)} className="ml-2 rounded-lg border border-slate-300 bg-white px-3 py-2 font-normal"><option value="all">All</option>{tab === "customers" ? <><option value="phone">Phone</option><option value="name">Name</option></> : <><option value="vin">VIN</option><option value="plate">License plate</option></>}</select></label>
    ...
    {visible.length === 0 ? <section className="mt-5 rounded-2xl border border-dashed border-slate-300 bg-white p-8 text-center"><h2 className="text-lg font-semibold">No duplicate groups match these filters</h2><p className="mt-2 text-sm text-slate-600">Try another record type or confidence level.</p></section> : <div className="mt-5 space-y-4">{visible.map((group) => {
      ...
      return <section key={group.id} className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        ...
        {open && <div className="overflow-x-auto border-t border-slate-200">{tab === "customers" ? <CustomerTable group={group as DuplicateGroup<CustomerDuplicate>} /> : <VehicleTable group={group as DuplicateGroup<VehicleDuplicate>} />}</div>}
      </section>;
    })}</div>}
  </>;
}
